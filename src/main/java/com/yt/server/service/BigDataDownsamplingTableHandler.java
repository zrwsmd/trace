...
public class BigDataDownsamplingTableHandler implements Callable<List<UniPoint>> {
    ...
    public BigDataDownsamplingTableHandler(String queryTable, Long reqStartTimestamp, Long reqEndTimestamp, JdbcTemplate jdbcTemplate, CountDownLatch countDownLatch, String varName, List<Map<String, String>> mapList) {
    ...
    public List<UniPoint> call() throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < 500; i++) {
                if (i == 0) {
                    Future<List<UniPoint>> future = pool.submit(new BigDataDownsamplingRowHandler(queryTable, reqStartTimestamp, reqStartTimestamp + shard * (i + 1), jdbcTemplate, innerCountDownLatch, varName,  mapList));
                    resultList.add(future);
                } else {
                    Future<List<UniPoint>> future = pool.submit(new BigDataDownsamplingRowHandler(queryTable, reqStartTimestamp + shard * (i) + 1, reqStartTimestamp + shard * (i + 1), jdbcTemplate, innerCountDownLatch, varName, mapList));
                    resultList.add(future);
                }
            }
            if ((reqEndTimestamp - reqStartTimestamp) % 500 != 0) {
                Future<List<UniPoint>> leftFuture = pool.submit(new BigDataDownsamplingRowHandler(queryTable, reqStartTimestamp + shard * 500 + 1, reqEndTimestamp, jdbcTemplate, innerCountDownLatch, varName, mapList));

...
* **尝试 Lag = 2 (平移 2 格) —— 为什么是它？**
    ...
    * **深度逻辑分析**：
        ...
        2. **数学计算 (点积)**：
            * 相关性的核心是乘积求和。
            * Idx 0 vs Idx 2: $(-4) \times (-4) = \mathbf{16}$ (**负负得正**，贡献极大正值)
            * Idx 1 vs Idx 3: $6 \times 6 = \mathbf{36}$ (**正正得正**，贡献极大正值)
            * Idx 2 vs Idx 4: $(-4) \times (-4) = \mathbf{16}$ (**负负得正**，贡献极大正值)
    * **结果**：
        * 对于 Lag=1，我们是在算 $(-4) \times 6 = -24$，正负抵消，得分很低。

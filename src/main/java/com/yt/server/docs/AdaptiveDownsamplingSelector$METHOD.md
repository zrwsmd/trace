...
1. **分段**：切分为前段（Segment 1, 500点）和后段（Segment 2, 500点）。
2. **复杂度计算**：
    * **Segment 1 (平缓)**：假设相邻点差值平均为 0.1。
      $$ \text{Complexity}_1 \approx (500 \times 0.1) + 1.0 = 51.0 $$
    * **Segment 2 (震荡)**：假设相邻点差值平均为 10.0。
      $$ \text{Complexity}_2 \approx (500 \times 10.0) + 1.0 = 5001.0 $$
    * **总复杂度**：
      $$ \text{Total} = 51.0 + 5001.0 = 5052.0 $$
3. **配额分配** (TotalTarget = 100)：
    * **Segment 1 配额**：
      $$ 100 \times \frac{51.0}{5052.0} \approx 100 \times 0.01 = 1 \text{ 点} $$
    * **Segment 2 配额**：
      $$ 100 \times \frac{5001.0}{5052.0} \approx 100 \times 0.99 = 99 \text{ 点} $$
4. **执行**：前段只采 1 个点，后段采 99 个点。
5. **效果**：所有宝贵的采样点都用在了刀刃上，完美还原了后半段的震荡波形。

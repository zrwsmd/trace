...
2. **计算重要性（曲率）**：对于中间的每一个点，计算其二阶差分（近似二阶导数）：
   $$ \text{Importance} = | \text{Next}_y - 2 × \text{Curr}_y + \text{Prev}_y | $$
...
* **一阶变化**：
    ...
    * 方向发生了 180度 大逆转！
* **二阶计算**：
  $$ |10 - 2 × 100 + 10| = |20 - 200| = |-180| \Rightarrow 180 $$
...
* **Idx 1**: $|10(\text{Idx0}) - 2 × 10(\text{Idx1}) + 50(\text{Idx2})| = |10 - 20 + 50| = |40| = $ **40**
* **Idx 2**: $|10(\text{Idx1}) - 2 × 50(\text{Idx2}) + 10(\text{Idx3})| = |10 - 100 + 10| = |-80| = $ **80** (
  弯曲度最大)
* **Idx 3**: $|50(\text{Idx2}) - 2 × 10(\text{Idx3}) + 10(\text{Idx4})| = |50 - 20 + 10| = |40| = $ **40**
...
1. **最小二乘法拟合**：通过计算 $x$ (索引) 和 $y$ (数值)
   的协方差与方差，找到一条误差最小的直线 $y = \text{slope} \cdot x + \text{intercept}$。
2. **提取参数**：
    * **Slope (斜率)**：表示数据的总体涨跌趋势（正数为涨，负数为跌，0为平）。
    * **Intercept (截距)**：趋势线的起始高度。
3. **计算残差 (Residuals)**：计算每个原始点与趋势线之间的垂直距离。
    * $\text{Residual}_i = y_i - (\text{slope} \cdot i + \text{intercept})$
...
* $\text{Intercept} = (34 - 1.5 × 3) / 3 = (34 - 4.5) / 3 = 29.5 / 3 \approx 9.833$
* **拟合直线**: $y = 1.5x + 9.833$

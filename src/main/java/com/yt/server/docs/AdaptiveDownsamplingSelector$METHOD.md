...
* **Idx 1**: |10(Idx0) - 2 × 10(Idx1) + 50(Idx2)| = |10 - 20 + 50| = |40| = **40**
* **Idx 2**: |10(Idx1) - 2 × 50(Idx2) + 10(Idx3)| = |10 - 100 + 10| = |-80| = **80** (
  弯曲度最大)
* **Idx 3**: |50(Idx2) - 2 × 10(Idx3) + 10(Idx4)| = |50 - 20 + 10| = |40| = **40**
...
1. **最小二乘法拟合**：通过计算 $x$ (索引) 和 $y$ (数值)
   的协方差与方差，找到一条误差最小的直线 y = slope × x + intercept。
2. **提取参数**：
...
假设我们有一组数据，看起来是一个简单的正弦波或三角波：
`[10, 20, 10, 0, 10, 20, 10, 0, 10, 20]`
...
* **观察**：
    ...
    * Idx 4: 10 (**回到起点**)
    * Idx 5: 20 (重复...)
* **直观周期**：每 4 个点重复一次。也就是 **Period = 4**。

**算法执行过程 (`detectPeriodicity`)**：

1. **标准化**：
   先减去均值（10），再除以标准差。
   变成类似：`[0, 1.4, 0, -1.4, 0, 1.4, 0, -1.4, ...]`
...
2. **扫描 Lag (假设 Step=1)**：
    ...
    * **Lag = 1 (平移 1 格)**：
      原序列: `0, 1.4, 0, -1.4, ...`
      平移后: `   0,   1.4, 0,    -1.4...`
      比较: `0 vs 1.4` (不匹配), `1.4 vs 0` (不匹配)...
      **结果**：相关性很低，甚至为负。

    * **Lag = 2 (平移 2 格)**：
      原序列: `0, 1.4, 0,   -1.4, ...`
      平移后: `        0,   1.4,  0...`
      比较: `0 vs 0`, `1.4 vs -1.4` (相反!)
      **结果**：波峰对波谷，**强负相关** (比如 -0.9)。

    * **Lag = 4 (平移 4 格)**：
      原序列 (Idx 4-7): `0, 1.4, 0, -1.4` ...
      平移后 (Idx 0-3): `0, 1.4, 0, -1.4` ...
      比较:
        * Idx 4 (0) vs Idx 0 (0) -> 匹配！
        * Idx 5 (1.4) vs Idx 1 (1.4) -> 匹配！
        * Idx 6 (0) vs Idx 2 (0) -> 匹配！
        * Idx 7 (-1.4) vs Idx 3 (-1.4) -> 匹配！
          **结果**：**相关系数极大 (接近 1.0)**。

3. **判定**：
   算法发现 Lag=4 时相关系数最大且超过阈值，因此输出：
    * `PeriodInfo.period = 4`
    * `PeriodInfo.strength = 0.99` (假设值)
...
* **尝试 Lag = 2 (平移 2 格)**
    ...
    * 平移项: `[-4, 6, -4]` (Idx 2-4)
    * 对比:
        * Idx 0 vs Idx 2: $(-4) \times (-4) = 16$ (匹配!)
        * Idx 1 vs Idx 3: $6 \times 6 = 36$ (匹配!)
        * Idx 2 vs Idx 4: $(-4) \times (-4) = 16$ (匹配!)
    * **结果**: 所有点完美重合，**相关系数 = 1.0 (最大值)**。
...
1. **原始数据** (`10, 22...`) $\xrightarrow{计算趋势}$ **趋势线** (`2x+14`) + **残差** (`-4, 6...`)
2. **残差** (`-4, 6...`) $\xrightarrow{自相关分析}$ **周期信息** (`Period=2`)

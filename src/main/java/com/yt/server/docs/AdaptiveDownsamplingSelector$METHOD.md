...
* **Idx 2**: |10(Idx1) - 2 × 50(Idx2) + 10(Idx3)| = |10 - 100 + 10| = |-80| = **80** (弯曲度最大)
...
**注意**: 由于起点和终点相同,一阶总变化在这个极端例子中为0,但代码实际计算的是**所有相邻点对**的变化:

```java
// 实际代码中的totalChange计算方式 (跨两步的总变化)
totalChange =|30-10|+|10-30|+|30-10|
        =20+20+20=60
```
...
totalChange(一阶总变化)  =60

noiseRatio =120/60=2.0
```

**结论**: **2.0** (>1.0) → 高噪声信号,二阶变化甚至是一阶变化的2倍!

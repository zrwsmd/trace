...
3. **动态分配配额**：根据每段复杂度占总复杂度的比例，分配目标点数 (`targetCount`)。
    * **公式**：
      $$ \text{SegmentTarget} = \text{TotalTarget} \times \frac{\text{SegmentComplexity}}{\text{TotalComplexity}} $$
...
2. **计算复杂度（关键步骤）**：
    ...
    * **第一段复杂度** ($500 \times 0.1 + 1.0$)：
      $$ \text{Complexity}_1 \approx 51.0 $$
    * **第二段复杂度** ($500 \times 10.0 + 1.0$)：
      $$ \text{Complexity}_2 \approx 5001.0 $$
    * **总复杂度**：
...
3. **分配配额（按需分配）**：
    ...
    * **第一段配额** (占约 1%)：
      $$ 100 \times \frac{51.0}{5052.0} \approx 1 \text{ 点} $$
    * **第二段配额** (占约 99%)：
      $$ 100 \times \frac{5001.0}{5052.0} \approx 99 \text{ 点} $$

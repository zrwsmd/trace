...
在数学和信号处理中，**二阶导数（Second Derivative）**反映的是信号的**曲率（Curvature）**或**加速度（Acceleration）**，即“变化的变化”。

#### 🔍 数学推导：为什么公式是 $y_{i+1} - 2y_i + y_{i-1}$？

我们可以通过**分步差分**来推导这个逻辑：

1. **第一步（求一阶导/速度 $v$）**：
   观察信号在两个相邻小区间内的变化：
    - 前一个区间的斜率（速度 $v_1$）：$v_1 = y_i - y_{i-1}$
    - 后一个区间的斜率（速度 $v_2$）：$v_2 = y_{i+1} - y_i$

2. **第二步（求二阶导/加速度 $a$）**：
   加速度定义为“速度的变化量”：
    - 加速度 $a = v_2 - v_1$
    - 代入公式：$a = (y_{i+1} - y_i) - (y_i - y_{i-1})$
    - 合并同类项所得：$\mathbf{a = y_{i+1} - 2y_i + y_{i-1}}$

这就是代码中 `Math.abs(y2 - 2 * y1 + y0)` 的由来。

#### 🌍 物理意义：

* **平滑/线性信号**：如果信号是直线，速度恒定，加速度为 **0**。如果信号是平缓曲线，加速度也很小。
* **噪声（随机抖动）**：噪声表现为信号在短时间内反复上下剧烈跳变。这意味着它的速度方向在频繁切换（比如一会儿是正，一会儿是负），导致“速度的变化”极大，从而产生巨大的二阶导数绝对值。

3. **判定指标**：
    * `smoothChange`：信号的“**总抖动量**”（所有二阶导数的绝对值求和）。
    * `totalChange`：信号的“**总位移量**”（跨越点的垂直距离求和）。
    * **结论**：`noiseRatio = 抖动量 / 位移量`。比值越高，说明信号的有效位移越少，随机抖动越多，信号质量越差。

#### 🔢 示例步进计算 (y = [1, 2, 3, 10, 11])

为了彻底理解这个公式，我们看这三个点是如何在循环中被处理的：

| 当前索引 (i) | 窗口数据 (y₀, y₁, y₂) | 一阶导 1 (y₁-y₀) | 一阶导 2 (y₂-y₁) | 二阶导 (Δv) = y₂-2y₁+y₀ | 物理含义          |
|:---------|:------------------|:--------------|:--------------|:---------------------|:--------------|
| **i=1**  | `[1, 2, 3]`       | 1             | 1             | **0**                | 斜率恒定，无加速度（平滑） |
| **i=2**  | `[2, 3, 10]`      | 1             | 7             | **+6**               | 向上突然加速（开始跳跃）  |
| **i=3**  | `[3, 10, 11]`     | 7             | 1             | **-6**               | 向上突然减速（跳跃结束）  |

**计算汇总：**

- **smoothChange** (抖动总量) = $|0| + |6| + |-6| = \mathbf{12}$
- **totalChange** (宏观跨度) = $|3-1| + |10-2| + |11-3| = 2 + 8 + 8 = \mathbf{18}$
- **noiseRatio** = $12 / 18 \approx \mathbf{0.667}$
...
2. **波动本质 (NormalizedVolatility=1.8, NoiseRatio=0.667)**：虽然信号在 3 到 10 之间有一个很大的跨度，但根据“噪声比”分析，其中约

...
### 5. 非线性权重配额分配 (Robin Hood 策略)
- **问题**: 简单的按波动指数比例分配点数，无法满足高频信号对采样率的渴求。正弦波需要的点数通常是普通趋势线的 10-20 倍。
- **解决方案**:
    - **极度偏袒的权重函数**: 引入 4 次方权重增长 `weight = Math.pow(v, 4.0)`。这使得波动较大的区域（如正弦波）能从平稳区域“掠夺”几乎所有的采样配额。
    - **精细化窗口**: 将 `WINDOW_SIZE` 缩减至 `256`，提高对局部高频特征的捕捉精度。
    - **激进阈值**: 将波动阈值降至 `0.5`，确保任何非平滑趋势的信号都能进入 Min-Max 处理路径。
    - **动态密度保障**: 对于检测到波动的窗口，强制分配至少 `1/8` 到 `1/4` 的原始数据点作为采样点（例如 256 个点最少采 32-64 个点），从底层杜绝混叠。
- **优势**: 彻底解决了极低采样率下高频信号“图像稀疏”或“乱码”的问题，保证了工业 PLC 等高频信号的视觉连续性。
